...
void CircleBres(int xc, int yc, int Radius) {
    ...
    y = Radius;
    p = 3 - 2 * Radius;
    draw4point(xc, yc, Radius, 0); //ve 4 diem dac biet
    while (x < y) {
        if (p < 0)
            p += 4 * x + 6;
        else {
        ...
        draw4point(xc, yc, y, x);
    }
    draw4point(xc, yc, y, y); // ve 4 diem phan giac x=y
}
...
int main(int argc, char ** argv) {
    ...
    glutCreateWindow("DEMO THUAT TOAN VE HINH TRON");
